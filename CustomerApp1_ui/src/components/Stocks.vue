...
const loadStocks = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/stock/stocks?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load stocks';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadStocks(currentPage.value + 1);
  }
};
...
const getStockIcon = (stock: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[stock.stockId % icons.length];
};

const viewStockDetails = (stock: any) => {
  router.push({ name: 'StockDetail', params: { id: stock.stockId } });
};
  ...
  <div class="stocks-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="stocks-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="stocks-content">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <h3 class="error-title">Failed to Load Stocks</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadStocks()" class="retry-button">
          Try Again
        </button>
      </div>
      ...
      <div v-else-if="filteredStocks.length === 0" class="empty-state">
        ...
        <h3 class="empty-title">No Stocks Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No stocks match your search criteria.' : 'No stocks found in the system.' }}
        </p>
      </div>

      <!-- Stocks Grid -->
      <div v-else-if="viewMode === 'cards'" class="stocks-grid">
        <div
        ...
        >
          ...
          <div class="stock-info">
            <h3 class="stock-name">{{ stock.name || 'Unknown Stock' }}</h3>
            ...
            <div class="stock-metadata">
              ...
              <div class="metadata-item">
                <span class="metadata-label">StockId:</span>
                <span class="metadata-value">{{ stock.stockId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">TickerSymbol:</span>
                <span class="metadata-value">{{ stock.tickerSymbol || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">CompanyName:</span>
                <span class="metadata-value">{{ stock.companyName || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">MarketValue:</span>
                <span class="metadata-value">{{ stock.marketValue || 'N/A' }}</span>
              </div>
      ...
      <!-- Stocks Table -->
      <div v-else-if="viewMode === 'table'" class="stocks-table-container">
        <table class="stocks-table">
          <thead>
            <tr>

              <th @click="handleSort('stockId')" class="sortable-header">
                StockId {{ getSortIcon('stockId') }}
              </th>
              <th @click="handleSort('tickerSymbol')" class="sortable-header">
                TickerSymbol {{ getSortIcon('tickerSymbol') }}
              </th>
              <th @click="handleSort('companyName')" class="sortable-header">
                CompanyName {{ getSortIcon('companyName') }}
              </th>
              <th @click="handleSort('marketValue')" class="sortable-header">
                MarketValue {{ getSortIcon('marketValue') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="stock in filteredStocks" :key="stock.stockId" class="stock-row">
              
              
                      <td class="stock-id-cell" @click="viewStockDetails(stock)">{{ stock.stockId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ stock.tickerSymbol }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ stock.companyName }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ stock.marketValue }}</span>
                </div>
              </td>
