...
const loadTraders = async (page = 0) => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/trader/traders?sortBy=${sortBy.value}&sortOrder=${sortOrder.value}&searchQuery=${searchQuery.value}&page=${page}&size=${pageSize.value}`);
    ...
    error.value = err.message || 'Failed to load traders';
  }
...
const nextPage = () => {
  if (currentPage.value < totalPages.value - 1) {
    loadTraders(currentPage.value + 1);
  }
};
...
const getTraderIcon = (trader: any) => {
  const icons = ['👤', '👨', '👩', '🧑', '👱', '🧔', '👨‍💼', '👩‍💼', '🧑‍💻', '👨‍🔬'];
  return icons[trader.traderId % icons.length];
};

const viewTraderDetails = (trader: any) => {
  router.push({ name: 'TraderDetail', params: { id: trader.traderId } });
};
...
<template>
  <div class="traders-page">
        ...
        <div class="header-content">
          <div class="header-info">
            <div class="title-row">
              ...
              <div class="traders-count-inline" v-if="!isLoading">
                ({{ totalElements }})
              </div>
            </div>
          </div>
    ...
    <main class="traders-content">
      <!-- Loading State -->
      <div v-if="isLoading" class="loading-state">
        <div class="loading-spinner-large"></div>
        <p class="loading-text">Loading traders...</p>
      </div>

      <!-- Error State -->
      <div v-else-if="error" class="error-state">
        <div class="error-icon">⚠️</div>
        <h3 class="error-title">Failed to Load Traders</h3>
        <p class="error-message">{{ error }}</p>
        <button @click="loadTraders()" class="retry-button">
          Try Again
        </button>
      </div>

      <!-- Empty State -->
      <div v-else-if="filteredTraders.length === 0" class="empty-state">
        <div class="empty-icon">👥</div>
        <h3 class="empty-title">No Traders Found</h3>
        <p class="empty-message">
          {{ searchQuery ? 'No traders match your search criteria.' : 'No traders found in the system.' }}
        </p>
      </div>

      <!-- Traders Grid -->
      <div v-else-if="viewMode === 'cards'" class="traders-grid">
        <div
        ...
        >
          <div class="trader-header">
            <div class="trader-icon">
              {{ getTraderIcon(trader) }}
            </div>
          </div>

          <div class="trader-info">
            <h3 class="trader-name">{{ trader.name || 'Unknown Trader' }}</h3>
            <p class="trader-description">
              Trader ID: {{ trader.traderId }}
            </p>
            
            <div class="trader-metadata">

              <div class="metadata-item">
                <span class="metadata-label">TraderId:</span>
                <span class="metadata-value">{{ trader.traderId || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">Name:</span>
                <span class="metadata-value">{{ trader.name || 'N/A' }}</span>
              </div>
              <div class="metadata-item">
                <span class="metadata-label">BrokerageFirm:</span>
                <span class="metadata-value">{{ trader.brokerageFirm || 'N/A' }}</span>
              </div>
      ...
      <!-- Traders Table -->
      <div v-else-if="viewMode === 'table'" class="traders-table-container">
        <table class="traders-table">
          <thead>
            <tr>

              <th @click="handleSort('traderId')" class="sortable-header">
                TraderId {{ getSortIcon('traderId') }}
              </th>
              <th @click="handleSort('name')" class="sortable-header">
                Name {{ getSortIcon('name') }}
              </th>
              <th @click="handleSort('brokerageFirm')" class="sortable-header">
                BrokerageFirm {{ getSortIcon('brokerageFirm') }}
              </th>

            </tr>
          </thead>
          <tbody>
            <tr v-for="trader in filteredTraders" :key="trader.traderId" class="trader-row">
              
              
                      <td class="trader-id-cell" @click="viewTraderDetails(trader)">{{ trader.traderId }}</td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ trader.name }}</span>
                </div>
              </td>
                              <td class="$field.name.camel-cell">
                <div class="$field.name.camel-info-table">
                  <span class="$field.name.camel-name-table">{{ trader.brokerageFirm }}</span>
                </div>
              </td>
...
.traders-table th:first-child,
.traders-table td:first-child {
  width: 140px;
  min-width: 140px;
  max-width: 140px;
}
